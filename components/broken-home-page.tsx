...
export default function BrokenHomePage() {
	...
	const displayedProducts = useMemo(() => {
		...
		if (isAdminUnlockQuery(raw)) {
			return [adminUnlockProductBroken];
		}
		const q = raw.toLowerCase();
		if (!q) return bakeryProducts;
		return bakeryProducts.filter((p) =>
			`${p.name} ${p.description}`.toLowerCase().includes(q),
		);
	}, [bakeryProducts, productSearch]);
			...
			<main className="container mx-auto p-3 relative">
				...
				<div className="mt-8 space-y-3">
					...
					<div className="relative max-w-xl">
						<label htmlFor="broken-product-search" className="sr-only">
							Søk etter varer
						</label>
						<Search
							className="pointer-events-none absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-pink-500"
							aria-hidden
						/>
						<input
							id="broken-product-search"
							type="search"
							value={productSearch}
							onChange={(e) => setProductSearch(e.target.value)}
							placeholder="Søk etter varer (virker sikkert)…"
							autoComplete="off"
							className="w-full rounded border-2 border-yellow-300 bg-pink-50 py-2 pl-9 pr-3 text-sm text-purple-700 animate-input-glitch placeholder:text-purple-400/80 focus:border-pink-400 focus:outline-none focus:ring-2 focus:ring-pink-200"
						/>
					</div>
					{productSearch.trim() !== "" && (
						<p
							className="text-xs text-purple-600 sm:text-sm"
							aria-live="polite"
						>
							{isAdminUnlockQuery(productSearch.trim())
								? "Administrativ nøkkel gjenkjent — én klassifisert oppføring (ikke i ordinær katalog)."
								: displayedProducts.length === 0
									? `Søkemotoren rapporterer: 404 egg for «${productSearch.trim()}».`
									: `${displayedProducts.length} vare${displayedProducts.length === 1 ? "" : "r"} funnet (tilsynelatende).`}
						</p>
					)}
				</div>
				...
				<section className="mt-4 grid grid-cols-1 gap-4">
					{displayedProducts.length === 0 ? (
						<div className="rounded-lg border-2 border-dashed border-pink-300 bg-yellow-50/80 px-4 py-8 text-center text-purple-700 decay-mold-border">
							<p className="text-sm font-bold glitch-text">
								Ingen treff i påskekatalogen
							</p>
							<p className="mt-2 text-xs text-blue-700 corrupt-text">
								Prøv «marsipan», «sjokolade» eller tøm søkefeltet — med mindre
								Skygg-haren har skjult varene.
							</p>
							<button
								type="button"
								onClick={() => setProductSearch("")}
								className="mt-4 text-sm font-bold text-pink-600 underline decoration-wavy"
							>
								Nullstill søk
							</button>
						</div>
						...
						<div
							...
							className={`bg-green-50 p-3 shadow-md relative overflow-hidden hover:animate-glitch-hover border-2 decay-mold-border ${
								product.id === ADMIN_UNLOCK_PRODUCT_ID
									? "border-emerald-500 ring-2 ring-amber-400/50"
									: "border-yellow-300"
							}`}
							style={{
								transform: `rotate(${product.id === ADMIN_UNLOCK_PRODUCT_ID ? 0 : productDecay[product.id]?.rot ?? 0}deg)`,
							}}
						>
							<div className="flex flex-col sm:flex-row gap-3">
								<div
									className={`h-32 w-32 flex items-center justify-center relative mx-auto sm:mx-0 border-2 ${
										product.id === ADMIN_UNLOCK_PRODUCT_ID
											? "border-amber-400 bg-gradient-to-br from-amber-100 via-yellow-50 to-purple-100"
											: "border-purple-200 bg-pink-100"
									}`}
								>
									{product.id === ADMIN_UNLOCK_PRODUCT_ID ? (
								...
								<div className="flex-1">
									...
									<div className="flex justify-between items-center">
										<div className="flex items-center gap-1">
											{product.id === ADMIN_UNLOCK_PRODUCT_ID ? (
												<span className="text-xs bg-emerald-200 px-2 py-1 rounded font-mono font-bold text-emerald-900 border border-emerald-500">
													OVERRIDE
												</span>
											) : (
							...
							{product.id === ADMIN_UNLOCK_PRODUCT_ID ? (
								<div className="absolute bottom-0 right-0 bg-amber-400 text-purple-950 text-xs px-2 py-1 font-mono font-bold">
									SHADOWBUNNY: NEUTRALISERT
								</div>
							) : (
